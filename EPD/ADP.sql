WITH DATA1 as (SELECT REC_ID,
     ...
     FACTOR_STREAM AS (SELECT DATA_CODE AS ITEM_CODE,
                       ...
                       WHERE LCI_ELEMENT_NAME IN
                             ('不可再生资源耗竭潜力（ADP）-化石燃料', '不可再生资源耗竭潜力（ADP）-矿物元素')),
     ...
     FACTOR_TRANSPORT1 AS (SELECT *
                           FROM BG00MAC102.T_ADS_WH_LCA_FACTOR_LIBRARY_LCI
                           WHERE YEAR = '2025'
                             AND LCI_ELEMENT_NAME IN ('不可再生资源耗竭潜力（ADP）-化石燃料',
                                                      '不可再生资源耗竭潜力（ADP）-矿物元素')
     ...
     FACTOR_SY AS (SELECT ITEM_CODE, LCI_ELEMENT_ID, LCI_ELEMENT_NAME, CAST(LCI_ELEMENT_VALUE AS DOUBLE) AS LCI_SY
                   ...
                   WHERE FLAG = 'SY'
                     AND LCI_ELEMENT_VALUE != 0),
     FACTOR_DISCH AS (SELECT A.ITEM_CODE, A.ITEM_NAME, B.DISCH_COEFF, B.HOTVALUE, B.HOTVALUE_UNIT
     ...
     RESULT1 AS (SELECT HEX(RAND())              AS REC_ID,
                 ...
                 FROM (SELECT *
                       FROM PROC_PRODUCT_LIST A
                                LEFT JOIN (SELECT DISTINCT LCI_ELEMENT_ID, LCI_ELEMENT_NAME FROM FACTOR_MAT) ON 1 = 1) A
                          LEFT JOIN C1_AGG
                                    ON A.PROC_KEY = C1_AGG.PROC_KEY AND A.LCI_ELEMENT_ID = C1_AGG.LCI_ELEMENT_ID
...
SELECT UUID, DATA_ITEM_NAME, LCI_ELEMENT_ID, LCI_ELEMENT_NAME, LCI_ELEMENT_VALUE
FROM BG00MAC102.T_ADS_WH_LCA_FACTOR_LIBRARY_LCI
WHERE YEAR = '2025';
;
