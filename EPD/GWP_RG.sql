WITH DATA1 as (SELECT REC_ID,
               ...
               WHERE COMPANY_CODE = 'RG'
                 AND BATCH_NUMBER = '20250120250420250630YS'),
     ...
     MATRIX_INV AS (SELECT PROC_KEY,
                    ...
                    WHERE BATCH_NUMBER = '20250120250420250630YS'
                      AND COMPANY_CODE = 'RG'
                      AND INV = 'Y'
                      AND UNIT_COST != 0),
     ...
     RESULT1 AS (SELECT HEX(RAND())              AS REC_ID,
                        '20250120250420250630YS' AS BATCH_NUMBER,
                        'RG'                     AS COMPANY_CODE,
                        A.PROC_KEY,
                        A.PROC_CODE,
                        A.PROC_NAME,
...
where BATCH_NUMBER = '20240120240120241204YS'
and COMPANY_CODE = 'WG';
...
where COMPANY_CODE = 'RG'
  and BATCH_NUMBER = '20250120250420250630YS'
  and LCA_PROC_CODE in ('BFBD', 'XFBC');

select * from T_ADS_FACT_LCA_SUBCLASS_RESULT_DIST_PARALLEL
where COMPANY_CODE = 'TM'
  and BATCH_NUMBER = 'MX202406'
and MAT_TRACK_NO = '20240412033504528173'
