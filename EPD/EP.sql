     ...
     FACTOR_STREAM AS (SELECT DATA_CODE AS ITEM_CODE,
                       ...
                       WHERE LCI_ELEMENT_NAME IN
                             ('不可再生资源耗竭潜力（ADP）-化石燃料', '不可再生资源耗竭潜力（ADP）-矿物元素',
                              '酸化潜力(AP)', '平流层臭氧层消耗潜力（ODP）', '富营养化潜力(EP)',
                              '对流层臭氧生成潜力(POCP)', '二次材料利用量', '危险固体废弃物处置',
                              '无危险固体废弃物处置', '放射性固体废弃物处置', '可再生二次燃料')),
                            ...
                            WHERE YEAR = '2025'
                              ...
                              AND LCI_ELEMENT_NAME IN
                                  ('不可再生资源耗竭潜力（ADP）-化石燃料', '不可再生资源耗竭潜力（ADP）-矿物元素',
                                   '酸化潜力(AP)', '平流层臭氧层消耗潜力（ODP）', '富营养化潜力(EP)',
                                   '对流层臭氧生成潜力(POCP)', '二次材料利用量', '危险固体废弃物处置',
                                   '无危险固体废弃物处置', '放射性固体废弃物处置', '可再生二次燃料')) B
                           ON A.UUID = B.UUID),
     ...
     RESULT1 AS (SELECT HEX(RAND())              AS REC_ID,
                 ...
                 FROM (SELECT *
                       FROM PROC_PRODUCT_LIST A
                                LEFT JOIN (SELECT DISTINCT LCI_ELEMENT_ID, LCI_ELEMENT_NAME FROM MAT) ON 1 = 1) A
...
select * from T_ADS_WH_LCA_MAT_DATA;
