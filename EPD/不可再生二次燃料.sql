WITH DATA1 as (SELECT REC_ID,
     ...
     FACTOR_FUEL1 AS (SELECT DATA_CODE AS ITEM_CODE,
                              STREAM_ID,
                              STREAM_NAME,
                              LCI_ELEMENT_ID,
                              LCI_ELEMENT_NAME,
                       ...
                       WHERE LCI_ELEMENT_NAME IN
                             ('不可再生二次燃料')),
     FACTOR_FUEL AS (SELECT A.ITEM_CODE,
                            STREAM_ID,
                            STREAM_NAME,
                            LCI_ELEMENT_ID,
                            LCI_ELEMENT_NAME,
                            LCI_ELEMENT_VALUE * B.HOTVALUE AS LCI_ELEMENT_VALUE
                     FROM FACTOR_FUEL1 A
                              LEFT JOIN (SELECT *
                                         FROM BG00MAC102.T_ADS_WH_LCA_MAT_DATA
                                         WHERE START_TIME = '2025'
                                           AND ORG_CODE = 'TA') B ON A.ITEM_CODE = B.ITEM_CODE),
     ...
     FACTOR_TRANSPORT1 AS (SELECT *
                           FROM BG00MAC102.T_ADS_WH_LCA_FACTOR_LIBRARY_LCI
                           WHERE YEAR = '2025'
                             AND LCI_ELEMENT_NAME IN ('不可再生二次燃料')
     ...
     RESULT1 AS (SELECT HEX(RAND())              AS REC_ID,
                 ...
                 FROM (SELECT *
                       FROM PROC_PRODUCT_LIST A
                                LEFT JOIN (SELECT DISTINCT LCI_ELEMENT_ID, LCI_ELEMENT_NAME FROM FACTOR_FUEL) ON 1 = 1) A
