WITH DATA1 as (SELECT REC_ID,
     ...
     UUID AS (SELECT *
              ...
              UNION
              (SELECT * FROM SY)),
     STREAM AS (SELECT UUID AS STREAM_ID,
                       NAME AS STREAM_NAME,
                       LCI_ELEMENT_ID,
                       LCI_ELEMENT_NAME,
                       LCI_ELEMENT_VALUE
     ...
     ITEM_BASE AS (SELECT DISTINCT ITEM_CODE, ITEM_NAME FROM DATA),
     ...
     FACTOR_TRANSPORT1 AS (SELECT *
                           FROM BG00MAC102.T_ADS_WH_LCA_FACTOR_LIBRARY_LCI
                           WHERE YEAR = '2025'
                             AND LCI_ELEMENT_NAME IN ('一次能源-不可再生能源:用作能量载体',
                                                      '一次能源-不可再生能源:用作原材料',
                                                      '一次能源-不可再生能源:合计',
                                                      '一次能源-可再生能源:用作能量载体',
                                                      '一次能源-可再生能源:用作原材料',
                            ...
                            WHERE YEAR = '2025'
                              ...
                              AND LCI_ELEMENT_NAME IN
                                  ('一次能源-不可再生能源:用作能量载体',
                                   ...
                                   '一次能源-可再生能源:合计')) B
                           ON A.UUID = B.UUID),
     FACTOR_MAT AS (SELECT DISTINCT A.ITEM_CODE,
                                    A.ITEM_NAME,
                                    B.FLAG,
                                    B.NAME AS MAT_NAME,
                                    B.LCI_ELEMENT_ID,
     ...
     FACTOR_FUEL AS (SELECT ITEM_CODE,
                            ITEM_NAME,
                            '356d4bf464794429945b469b82afc90b' AS LCI_ELEMENT_ID,
                            '一次能源-不可再生能源:合计'       AS LCI_ELEMENT_NAME,
                            HOTVALUE                           AS LCI_ELEMENT_VALUE
                     FROM (SELECT *
     ...
     C1_DIST AS (SELECT PROC_KEY,
                        ...
                        CASE
                            ...
                            ELSE -A.UNIT_COST * B.LCI_ELEMENT_VALUE END AS LOAD
                 FROM DATA A
                          INNER JOIN FACTOR_FUEL B ON A.ITEM_CODE = B.ITEM_CODE),
     ...
     RESULT1 AS (SELECT HEX(RAND())              AS REC_ID,
                 ...
                 FROM (SELECT *
                       FROM PROC_PRODUCT_LIST A
                                LEFT JOIN (SELECT DISTINCT LCI_ELEMENT_ID, LCI_ELEMENT_NAME FROM MAT)
                                          ON 1 = 1) A
...
SELECT *
FROM FACTOR_MAT;
-- WHERE LCI_ELEMENT_NAME = '一次能源-不可再生能源:合计'
;
...
from (SELECT A.*,
             CASE
                 WHEN ITEM_CAT_NAME IN ('产品', '副产品') THEN -A.UNIT_COST * B.HOTVALUE
                 ELSE A.UNIT_COST * B.HOTVALUE END AS LOAD
      FROM DATA A
               JOIN (SELECT *
                     FROM BG00MAC102.T_ADS_WH_LCA_MAT_DATA
                     WHERE START_TIME = '2025'
      ...
      ORDER BY A.PROC_KEY, ITEM_CAT_CODE) A
         left join (select proc_key,
                           proc_name,
                           product_name,
                           c_cycle,
                           g_cycle,
                           ...
                           g1
                    from RESULT_DIFF
                    where LCI_ELEMENT_NAME = '一次能源-不可再生能源:合计') B on A.PROC_KEY = B.PROC_KEY




select * from T_ADS_WH_LCA_FACTOR_LIBRARY_LCI
where LCI_ELEMENT_ID = 'b139f8f5707649f1b0fefc2aa5b47a6b';
