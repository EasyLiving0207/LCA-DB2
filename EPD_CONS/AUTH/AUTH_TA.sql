...
order by LCI_ELEMENT_CODE;

select *
from T_ADS_FACT_LCA_PROC_DATA_0002_DT
where COMPANY_CODE = 'TA'
  and BATCH_NUMBER = '20240120241220251118YS'
...
where COMPANY_CODE = 'TA'
  and BATCH_NUMBER = '20240120241220251118YS_CONS'
  and (LCA_PROC_CODE like 'CO%' or LCA_PROC_CODE like 'BF%' or
       LCA_PROC_CODE in ('1LDG', '1YLT', '2YLT', 'LDZQ'))
...
from (select distinct LCA_DATA_ITEM_CODE, LCA_DATA_ITEM_NAME
      from T_ADS_FACT_LCA_PROC_DATA_0002_DT
      where COMPANY_CODE = 'TA'
        and BATCH_NUMBER = '20240120241220251118YS') A
...
from (select *
      ...
      where COMPANY_CODE = 'TA'
        and BATCH_NUMBER = '20240120241220251118YS_CONS') A
         join (select *
               from T_ADS_WH_LCA_EPD_CONS_FACTOR_VERSION
               where VERSION = 'EN15804_Ecoinvent3.11'
...
where MAT_TRACK_NO = '20240101131148921891'
...
CALL BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_AUTH(
        ...
        '20240120241220251118YS_CONS',--大类结果批次号（批次号 + _CONS）
        'T_ADS_FACT_LCA_HP0001_2024_TEST',--活动数据表
        'T_ADS_FACT_LCA_SUBCLASS_EPD_CONS_MONTH_RESULT_TA_HP0001_AUTH',
        'T_ADS_FACT_LCA_SUBCLASS_EPD_CONS_MONTH_RESULT_TA_HP0001_AUTH_DIST'
     );
...
select company_code,
       ...
       c_insite  as 厂内负荷,
       c_outsite as 厂外负荷
from T_ADS_FACT_LCA_SUBCLASS_EPD_CONS_MONTH_RESULT_TA_HP0001_AUTH
where MAIN_CAT_BATCH_NUMBER = '20240120241220251118YS_CONS'
  and MAT_TRACK_NO = '20240101131148921891'
...
select update_date,
       ...
       c4                     as 回用抵扣负荷,
       c5                     as 运输负荷
from T_ADS_FACT_LCA_SUBCLASS_EPD_CONS_MONTH_RESULT_TA_HP0001_AUTH_DIST
where MAT_TRACK_NO = '20240101131148921891'
...
CALL BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_AUTH(
        ...
        'EN15804_Ecoinvent3.11',--因子库版本 默认EN15804_Ecoinvent3.11
        'T_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_RESULT',--大类结果表
        '20240120241220251118YS_GREEN_CONS',--大类结果批次号（批次号 + _CONS）
        'T_ADS_FACT_LCA_HP0001_2025_NEW_1105_GREEN',--活动数据表
        'T_ADS_FACT_LCA_SUBCLASS_EPD_CONS_MONTH_RESULT_HP0001_2025_NEW_1105_GREEN',
        'T_ADS_FACT_LCA_SUBCLASS_EPD_CONS_MONTH_RESULT_HP0001_2025_NEW_1105_GREEN_DIST'
     );
...
select company_code,
       ...
       c_insite  as 厂内负荷,
       c_outsite as 厂外负荷
from T_ADS_FACT_LCA_SUBCLASS_EPD_CONS_MONTH_RESULT_HP0001_2025_NEW_1105_GREEN
order by LCI_ELEMENT_CODE, FAMILY_CODE, MAT_SEQ_NO;
...
select update_date,
       ...
       c4                     as 回用抵扣负荷,
       c5                     as 运输负荷
from T_ADS_FACT_LCA_SUBCLASS_EPD_CONS_MONTH_RESULT_HP0001_2025_NEW_1105_GREEN_DIST
order by LCI_ELEMENT_CODE, FAMILY_CODE, MAT_SEQ_NO, TYPE_CODE;
