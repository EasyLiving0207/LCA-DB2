CREATE OR REPLACE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_CALC_VER2(IN V_COMPANY_CODE VARCHAR(4),
    ...
    SPECIFIC P_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_CALC_VER2
    LANGUAGE SQL
    NOT DETERMINISTIC
    EXTERNAL ACTION
    MODIFIES SQL DATA
...
BEGIN
    ...
    DECLARE V_ROUTINE_NAME VARCHAR(128) DEFAULT 'P_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_CALC_VER2'; --存储过程名
    ...
    DECLARE V_TMP_TAB VARCHAR(50) DEFAULT 'T_ADS_TEMP_LCA_MAIN_CAT_EPD_CONS_CALC_VER2'; --临时表名
    DECLARE V_PNAME VARCHAR(50) DEFAULT 'P_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_CALC_VER2'; --存储过程名
    DECLARE V_QUERY_STR CLOB(1 M); --查询SQL
    DECLARE V_TMP_NAME VARCHAR(128);
    DECLARE V_MAIN_BATCH_NUMBER_CONS VARCHAR(100);
    ...
    SET V_MAIN_BATCH_NUMBER_CONS = CONCAT(V_MAIN_CAT_BATCH_NUMBER, '_CONS');
    ...
    SET V_QUERY_STR = 'SELECT REC_ID,
                              ...
                              CASE
                                  ...
                                  END
                                                                               AS UNIT
                       FROM BG00MAC102.T_ADS_FACT_LCA_PROC_DATA_CONS
    ...
    CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'TEMP_DATA_NORM', V_QUERY_STR);
    ...
    SET V_QUERY_STR = 'SELECT DISTINCT PROC_KEY,
                                       ...
                                       ITEM_NAME AS PRODUCT_NAME,
                                       VALUE     AS PRODUCT_VALUE
                           FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_TEMP_DATA_NORM
                           WHERE ITEM_CAT_NAME = ''产品''';
    CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'PROC_PRODUCT_LIST_NORM', V_QUERY_STR);


    SET V_QUERY_STR = 'SELECT A.*,
                              B.PRODUCT_VALUE,
                              CAST(A.VALUE AS DOUBLE) / CAST(B.PRODUCT_VALUE AS DOUBLE) AS UNIT_COST
                      FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_TEMP_DATA_NORM A
                               JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PROC_PRODUCT_LIST_NORM B
                                    ON A.PROC_KEY = B.PROC_KEY';
    CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'DATA_NORM', V_QUERY_STR);


    SET V_QUERY_STR = 'SELECT DISTINCT ITEM_CODE, ITEM_NAME
         FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATA_NORM';
    ...
    SET V_QUERY_STR = 'SELECT PROC_KEY,
                       ...
                       WHERE BATCH_NUMBER = ''' || V_MAIN_BATCH_NUMBER_CONS || '''
    ...
    SET V_QUERY_STR = 'SELECT DISTINCT CONS_LCI_ELEMENT_NAME AS LCI_ELEMENT_NAME ' ||
                      'FROM BG00MAC102.T_ADS_FACT_LCA_EPD_LCI_ELEMENT_NORM_CONS_MAP ' ||
                      'WHERE CONS_LCI_ELEMENT_NAME IS NOT NULL ' ||
                      'ORDER BY CONS_LCI_ELEMENT_NAME';
    ...
    SET V_QUERY_STR = '
            SELECT A.ITEM_CODE AS ITEM_CODE,
                   ...
                   ''climate change - global warming potential (GWP100)'' AS LCI_ELEMENT_NAME,
                   B.DISCH_COEFF AS LCI_ELEMENT_VALUE
            FROM (SELECT ITEM_CODE, ITEM_NAME FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ITEM_BASE
                      WHERE ITEM_CODE NOT IN (SELECT DISTINCT PRODUCT_CODE FROM ' || V_TMP_SCHEMA || '.' ||
                      V_TMP_TAB || '_PROC_PRODUCT_LIST)) A
                  JOIN
                 (SELECT DISTINCT *
            ...
            SELECT A.ITEM_CODE AS ITEM_CODE,
                   NULL AS LCI_ELEMENT_ID,
                   ''climate change: fossil - global warming potential (GWP100)''       AS LCI_ELEMENT_NAME,
                   B.DISCH_COEFF AS LCI_ELEMENT_VALUE
            FROM (SELECT ITEM_CODE, ITEM_NAME FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ITEM_BASE
                      WHERE ITEM_CODE NOT IN (SELECT DISTINCT PRODUCT_CODE FROM ' || V_TMP_SCHEMA || '.' ||
                      V_TMP_TAB || '_PROC_PRODUCT_LIST)) A
...
--             SELECT A.ITEM_CODE AS ITEM_CODE,
--                    NULL AS LCI_ELEMENT_ID,
--                    ''一次能源-不可再生能源:合计'' AS LCI_ELEMENT_NAME,
...
--             UNION
--             SELECT A.ITEM_CODE AS ITEM_CODE,
--                    ''86dcfbcfd8bf4dac8bf309d9d28a4bda'' AS LCI_ELEMENT_ID,
--                    ''一次能源-不可再生能源:用作能量载体''       AS LCI_ELEMENT_NAME,
--                    B.HOTVALUE AS LCI_ELEMENT_VALUE
--             FROM (SELECT ITEM_CODE, ITEM_NAME FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ITEM_BASE) A
--                  JOIN
--                  (SELECT DISTINCT *
...
--                    A.LCI_ELEMENT_ID,
--                    A.LCI_ELEMENT_NAME,
--                    A.LCI_ELEMENT_VALUE * B.HOTVALUE * 1000 AS LCI_ELEMENT_VALUE
--             FROM
--             (SELECT DATA_CODE AS ITEM_CODE,
--                    LCI_ELEMENT_ID,
--                    LCI_ELEMENT_NAME,
--                    LCI_ELEMENT_VALUE
--             FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_LCI_STREAM
--             WHERE LCI_ELEMENT_NAME = ''不可再生二次燃料'') A
--             LEFT JOIN (SELECT *
--               FROM BG00MAC102.T_ADS_WH_LCA_MAT_DATA
--               WHERE START_TIME = ''' || V_FACTOR_YEAR || '''
--                 AND ORG_CODE = ''' || V_COMPANY_CODE || ''') B ON A.ITEM_CODE = B.ITEM_CODE';
--     CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'FACTOR_SECOND_FUEL', V_QUERY_STR);
    ...
    SET V_QUERY_STR = '
            ...
            SELECT *
            FROM
            ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_GWP';
    ...
    SET V_QUERY_STR = 'SELECT PROC_KEY,
                       ...
                       FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATA A
                                INNER JOIN (SELECT *
                                            FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_GWP) B
                                           ON A.ITEM_CODE = B.ITEM_CODE
                       UNION
                       SELECT PROC_KEY,
                              PROC_CODE,
    ...
    SET V_QUERY_STR = 'SELECT A.PROC_KEY           AS SOURCE_PROC_KEY,
                       ...
                       AND A.LCI_ELEMENT_NAME = ''climate change - global warming potential (GWP100)''';
    ...
    SET V_QUERY_STR = 'WITH C1_CYCLE AS (SELECT A.PROC_KEY           AS SOURCE_PROC_KEY,
                                         ...
                                         FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C1_DIST A
                                                  CROSS JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_MATRIX_INV B
                                         WHERE A.PROC_KEY = B.SOURCE_PROC_KEY)
                        SELECT TARGET_PROC_KEY AS PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME, SUM(COST) AS C1
                        FROM C1_CYCLE
    ...
    SET V_QUERY_STR = 'WITH C2_CYCLE AS (SELECT A.PROC_KEY           AS SOURCE_PROC_KEY,
                                         ...
                                         FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C2_DIST A
                                                  CROSS JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_MATRIX_INV B
                                         WHERE A.PROC_KEY = B.SOURCE_PROC_KEY)
                        SELECT TARGET_PROC_KEY AS PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME, SUM(COST) AS C2
                        FROM C2_CYCLE
                        GROUP BY TARGET_PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME';
    ...
    SET V_QUERY_STR = 'WITH C3_CYCLE AS (SELECT A.PROC_KEY           AS SOURCE_PROC_KEY,
                                         ...
                                         FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C3_DIST A
                                                  CROSS JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_MATRIX_INV B
                                         WHERE A.PROC_KEY = B.SOURCE_PROC_KEY)
                        SELECT TARGET_PROC_KEY AS PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME, SUM(COST) AS C3
                        FROM C3_CYCLE
                        GROUP BY TARGET_PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME';
    ...
    SET V_QUERY_STR = 'WITH C4_CYCLE AS (SELECT A.PROC_KEY           AS SOURCE_PROC_KEY,
                                         ...
                                         FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C4_DIST A
                                                  CROSS JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_MATRIX_INV B
                                         WHERE A.PROC_KEY = B.SOURCE_PROC_KEY)
                        SELECT TARGET_PROC_KEY AS PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME, SUM(COST) AS C4
                        FROM C4_CYCLE
                        GROUP BY TARGET_PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME';
    ...
    SET V_QUERY_STR = 'WITH C5_CYCLE AS (SELECT A.PROC_KEY           AS SOURCE_PROC_KEY,
                                         ...
                                         FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C5_DIST A
                                                  CROSS JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_MATRIX_INV B
                                         WHERE A.PROC_KEY = B.SOURCE_PROC_KEY)
                        SELECT TARGET_PROC_KEY AS PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME, SUM(COST) AS C5
                        FROM C5_CYCLE
                        GROUP BY TARGET_PROC_KEY, LCI_ELEMENT_ID, LCI_ELEMENT_NAME';
    ...
    SET V_QUERY_STR = 'WITH RESULT1 AS (SELECT HEX(RAND())                      AS REC_ID,
                                      ...
                                      FROM (SELECT *
                                            ...
                                            LEFT JOIN (SELECT DISTINCT LCI_ELEMENT_NAME FROM ' || V_TMP_SCHEMA || '.' ||
                      V_TMP_TAB || '_LCI_LIST) ON 1 = 1) A
                                               LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C1_AGG C1_AGG ON A.PROC_KEY = C1_AGG.PROC_KEY AND A.LCI_ELEMENT_NAME = C1_AGG.LCI_ELEMENT_NAME
                                               LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C2_AGG C2_AGG ON A.PROC_KEY = C2_AGG.PROC_KEY AND A.LCI_ELEMENT_NAME = C2_AGG.LCI_ELEMENT_NAME
                                               LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C3_AGG C3_AGG ON A.PROC_KEY = C3_AGG.PROC_KEY AND A.LCI_ELEMENT_NAME = C3_AGG.LCI_ELEMENT_NAME
                                               LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C4_AGG C4_AGG ON A.PROC_KEY = C4_AGG.PROC_KEY AND A.LCI_ELEMENT_NAME = C4_AGG.LCI_ELEMENT_NAME
                                               LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C5_AGG C5_AGG ON A.PROC_KEY = C5_AGG.PROC_KEY AND A.LCI_ELEMENT_NAME = C5_AGG.LCI_ELEMENT_NAME
                                      ORDER BY A.PROC_KEY, A.LCI_ELEMENT_NAME)
                      SELECT *,
    ...
    --     SET V_QUERY_STR = 'UPDATE ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT
--                         SET C1_DIRECT = 0,
--                             C2_BP     = 0,
--                             C3_OUT    = 0,
--                             C4_BP_NEG = 0,
...
--                             C_OUTSITE = 0,
--                             C_CYCLE   = 0
--                         WHERE LCI_ELEMENT_NAME IN (''一次能源-不可再生能源:用作原材料'', ''一次能源-可再生能源:用作原材料'')';
