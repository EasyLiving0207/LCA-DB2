CREATE OR REPLACE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_CALC(IN V_COMPANY_CODE VARCHAR(4),
    ...
    SPECIFIC P_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_CALC
    LANGUAGE SQL
    NOT DETERMINISTIC
    EXTERNAL ACTION
    MODIFIES SQL DATA
...
BEGIN
    ...
    DECLARE V_ROUTINE_NAME VARCHAR(128) DEFAULT 'P_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_CALC'; --存储过程名
    ...
    DECLARE V_TMP_TAB VARCHAR(50) DEFAULT 'T_ADS_TEMP_LCA_MAIN_CAT_EPD_CONS_CALC'; --临时表名
    DECLARE V_PNAME VARCHAR(50) DEFAULT 'P_ADS_FACT_LCA_MAIN_CAT_EPD_CONS_CALC'; --存储过程名
...
--     CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'FACTOR_TRANSPORT', V_QUERY_STR);
    ...
    SET V_QUERY_STR = 'SELECT PROC_KEY,
                       ...
                       FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATA A
                                INNER JOIN (SELECT *
                                    ...
                                    ON A.ITEM_CODE = B.ITEM_CODE';
    CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'C1_DIST_CONS', V_QUERY_STR);
    ...
    SET V_QUERY_STR = 'SELECT PROC_KEY,
                              ...
                              CASE
                                  WHEN ITEM_CAT_CODE < ''04'' THEN A.UNIT_COST * B.LCI_ELEMENT_VALUE
                                  ELSE -ABS(A.UNIT_COST * B.LCI_ELEMENT_VALUE) END AS LOAD
                       FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATA_NORM A
                                INNER JOIN (SELECT *
                                            FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_FUEL) B
                                           ON A.ITEM_CODE = B.ITEM_CODE
                       UNION
                       ...
                       FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATA_NORM A
                                INNER JOIN (SELECT *
                                            FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_EP) B
                                    ON A.ITEM_CODE = B.ITEM_CODE';
    CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'C1_DIST_NORM', V_QUERY_STR);


    IF V_COMPANY_CODE = 'TA' THEN
        SET V_QUERY_STR = 'WITH DATA_NORM AS (SELECT *
                             ...
                             DIST_NORM AS (SELECT *
                                           FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C1_DIST_NORM
                                           WHERE LOAD IS NOT NULL),
                             ...
                             SUM_LOAD AS (SELECT *
                                          ...
                                          FROM LDZQ),
                             REST_LOAD AS (SELECT *
                                           FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C1_DIST_CONS
                                           WHERE LOAD IS NOT NULL
    ...
    ELSEIF V_COMPANY_CODE = 'ZG' THEN
        SET V_QUERY_STR = 'WITH DATA_NORM AS (SELECT *
                                 ...
                                 DIST_NORM AS (SELECT *
                                               FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_C1_DIST_NORM
                                               WHERE LOAD IS NOT NULL),
                                 CO_DIST AS (SELECT * FROM DIST_NORM WHERE PROC_CODE IN (''LT04'', ''XABC'')),
                                 COKE_LOAD AS (SELECT PROC_KEY,
                                                      PROC_CODE,
                                                      PROC_NAME,
                                                      PRODUCT_NAME,
                                 ...
                                 COG_LOAD AS (SELECT A.PROC_KEY,
                                          ...
                                          FROM (SELECT *
                                                FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PROC_PRODUCT_LIST
                                                WHERE PROC_CODE = ''COMQ'') A
                                                   JOIN (SELECT * FROM DATA_NORM WHERE PROC_CODE IN (''LT04'', ''XABC'')) B ON 1 = 1),
                                 CO_STEAM_LOAD AS (SELECT A.PROC_KEY,
                                                        A.PROC_CODE,
                                                        A.PROC_NAME,
                                                        A.PRODUCT_NAME,
                                                        ITEM_CAT_CODE,
                                                        ...
                                                        LCI_ELEMENT_NAME,
                                                        LCI_ELEMENT_VALUE,
                                                        CAST(B.LOAD AS DOUBLE) / CAST(A.PRODUCT_VALUE AS DOUBLE) * 0.0401  AS LOAD
                                                 FROM (SELECT *
                                                       FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PROC_PRODUCT_LIST
                                                       WHERE PROC_CODE = ''COZQ'') A
                                                          JOIN (SELECT * FROM DATA_NORM WHERE PROC_CODE IN (''LT04'', ''XABC'')) B ON 1 = 1),
                                 BF_DIST AS (SELECT * FROM DIST_NORM WHERE PROC_CODE IN (''LT05'', ''LT06'', ''LT10'')),
                                 IRON_LOAD AS (SELECT PROC_KEY,
                                                      PROC_CODE,
                                                      PROC_NAME,
                                                      PRODUCT_NAME,
                                 ...
                                 BFG_LOAD AS (SELECT A.PROC_KEY,
                                              ...
                                              FROM (SELECT *
                                                    FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PROC_PRODUCT_LIST
                                                    WHERE PROC_CODE = ''LTMQ'') A
                                                       JOIN (SELECT * FROM DATA_NORM WHERE PROC_CODE IN (''LT05'', ''LT06'', ''LT10'')) B
                                                            ON 1 = 1),
                                 TRT_LOAD AS (SELECT A.PROC_KEY,
                                                     A.PROC_CODE,
                                                     A.PROC_NAME,
                                              ...
                                              FROM (SELECT *
                                                    FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PROC_PRODUCT_LIST
                                                    WHERE PROC_CODE = ''LTDL'') A
                                                       JOIN (SELECT * FROM DATA_NORM WHERE PROC_CODE IN (''LT05'', ''LT06'', ''LT10'')) B
                                                            ON 1 = 1),
                                 BOF_DIST AS (SELECT * FROM DIST_NORM WHERE PROC_CODE IN (''LG04'')),
                                 STEEL_LOAD AS (SELECT PROC_KEY,
                                                       PROC_CODE,
                                                       PROC_NAME,
                                                       PRODUCT_NAME,
                                 ...
                                 LDG_LOAD AS (SELECT A.PROC_KEY,
                                              ...
                                              FROM (SELECT *
                                                    FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PROC_PRODUCT_LIST
                                                    WHERE PROC_CODE = ''LGMQ'') A
                                                       JOIN (SELECT * FROM DATA_NORM WHERE PROC_CODE IN (''LG04'')) B ON 1 = 1),
                                 BOF_STEAM_LOAD AS (SELECT A.PROC_KEY,
                                                     A.PROC_CODE,
                                                     A.PROC_NAME,
                                                     A.PRODUCT_NAME,
                                                     ITEM_CAT_CODE,
