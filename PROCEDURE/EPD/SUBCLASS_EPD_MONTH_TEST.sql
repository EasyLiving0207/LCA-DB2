CREATE OR REPLACE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_EPD_NORM_CALC(IN V_COMPANY_CODE VARCHAR(4),
    ...
    SPECIFIC P_ADS_FACT_LCA_SUBCLASS_EPD_NORM_CALC
    LANGUAGE SQL
    NOT DETERMINISTIC
    EXTERNAL ACTION
    MODIFIES SQL DATA
...
BEGIN
    ...
    DECLARE V_ROUTINE_NAME VARCHAR(128) DEFAULT 'P_ADS_FACT_LCA_SUBCLASS_EPD_NORM_CALC'; --存储过程名
    ...
    DECLARE V_TMP_TAB VARCHAR(50) DEFAULT 'T_ADS_TEMP_LCA_SUBCLASS_EPD_NORM_CALC'; --临时表名
    ...
    IF V_START_MONTH IS NOT NULL THEN
        SET V_QUERY_STR = 'SELECT UPDATE_DATE,
                                  ...
                                  C4,
                                  C5
                           FROM ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_TAB_NAME || ' A
                ...
                FROM (SELECT A.*,
                      ...
                      FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_INPUT_RESOURCE A
                               JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_TRANSPORT B
                                    ON A.ITEM_CODE = B.ITEM_CODE)
                GROUP BY UPDATE_DATE, INDEX_CODE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO,
                       MAT_WT, MAT_STATUS, FAMILY_CODE, UNIT_CODE, UNIT_NAME,
        ...
        DO
            ...
            SET V_QUERY_STR = 'INSERT INTO SESSION.DIST_BUFFER
                                   ...
                                   GROUP BY im.UPDATE_DATE, im.INDEX_CODE, im.MAT_NO, im.MAT_TRACK_NO, im.MAT_SEQ_NO,
                                            im.MAT_WT, im.MAT_STATUS, im.FAMILY_CODE, im.UNIT_CODE, im.UNIT_NAME,
                                            im.PRODUCT_CODE, im.PRODUCT_NAME, im.PRODUCT_VALUE, dist.LCI_ELEMENT_CODE';
            PREPARE stmt FROM V_QUERY_STR;
            EXECUTE stmt;
            ...
            INSERT INTO SESSION.RESULT_BUFFER (UPDATE_DATE, INDEX_CODE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO,
                                               MAT_WT, MAT_STATUS, FAMILY_CODE, UNIT_CODE,
            ...
            GROUP BY UPDATE_DATE, INDEX_CODE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, MAT_WT, MAT_STATUS, FAMILY_CODE,
                     UNIT_CODE, UNIT_NAME, PRODUCT_CODE, PRODUCT_NAME, PRODUCT_VALUE, LCI_ELEMENT_CODE;
            ...
            SET V_QUERY_STR = 'INSERT INTO ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_TAB_NAME || ' (REC_ID, SUBCLASS_TAB_NAME, COMPANY_CODE, MAIN_CAT_BATCH_NUMBER,
                                                                             FACTOR_VERSION, UPDATE_DATE, INDEX_CODE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, MAT_WT, MAT_STATUS, FAMILY_CODE,
                       ...
                       GROUP BY UPDATE_DATE, INDEX_CODE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, MAT_WT,
                                MAT_STATUS, FAMILY_CODE, UNIT_CODE, UNIT_NAME,
