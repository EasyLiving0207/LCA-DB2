CREATE OR REPLACE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_GWP_AGG(
    ...
    SPECIFIC P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_GWP_AGG
    LANGUAGE SQL
    NOT DETERMINISTIC
    EXTERNAL ACTION
    MODIFIES SQL DATA
...
BEGIN
    ...
    DECLARE V_ROUTINE_NAME VARCHAR(128) DEFAULT 'P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_GWP_AGG'; --存储过程名
    ...
    DECLARE V_TMP_TAB VARCHAR(50) DEFAULT 'T_ADS_TEMP_LCA_SUBCLASS_SEQ_CALC_GWP_AGG'; --临时表名
    DECLARE V_PNAME VARCHAR(50) DEFAULT 'P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_GWP_AGG'; --存储过程名
...
--     DECLARE CONTINUE HANDLER FOR NOT FOUND SET DONE = 1;
    ...
    SET v_query_str = '
    CREATE TABLE ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT_AGG
    (
        SUBCLASS_TAB_NAME     VARCHAR(31),
...
--     SET V_QUERY_STR = '
--         INSERT INTO ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT_AGG
...
--                0                                               AS C2,
--                0                                               AS C3,
--                COALESCE(-A.UNIT_COST * B.LCI_ELEMENT_VALUE, 0) AS C4,
--                0                                               AS C5
--         FROM (SELECT *
--               FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PRODUCT_BY_PRODUCT
--                        LEFT JOIN (SELECT DISTINCT LCI_ELEMENT, LCI_ELEMENT_NAME
--                                   FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ENERGY_RESULT) ON 1 = 1) A
--                  INNER JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_GWP B
--                            ON A.ITEM_CODE = B.ITEM_CODE AND A.LCI_ELEMENT = B.LCI_ELEMENT
--         UNION
--         SELECT A.*,
                    ...
                    FROM (SELECT A.*
                          FROM (SELECT *
                             ...
                             INNER JOIN (SELECT SUBCLASS_TAB_NAME,
                                          ...
                                          SUM(C4) AS C4,
                                          SUM(C5) AS C5
                                   FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT_AGG
                                   GROUP BY SUBCLASS_TAB_NAME,
                                               BATCH_NUMBER,
                                               MAT_NO,
                                               MAT_TRACK_NO,
