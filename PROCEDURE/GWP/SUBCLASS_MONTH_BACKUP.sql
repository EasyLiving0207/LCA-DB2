CREATE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_MONTH(IN V_COMPANY_CODE VARCHAR(4),
...
--     DECLARE CONTINUE HANDLER FOR NOT FOUND SET DONE = 1;
    ...
    SET V_QUERY_STR = '
        SELECT A.* FROM (SELECT BATCH_NUMBER,
               UPDATE_DATE,
               MAT_TRACK_NO,
               MAT_NO,
               MAT_SEQ_NO,
    ...
    IF V_START_MONTH IS NOT NULL THEN
        SET V_QUERY_STR = '
            INSERT INTO ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT_DIST (SUBCLASS_TAB_NAME, BATCH_NUMBER, UPDATE_DATE, MAT_NO,
                                                                MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE, UNIT_CODE,
                                                                UNIT_NAME, PRODUCT_CODE, PRODUCT_NAME, TYPE_CODE,
                                                                TYPE_NAME, ITEM_CODE, ITEM_NAME, VALUE, UNITM_AC,
                                                                PRODUCT_VALUE, UNIT_COST, C1, C2, C3, C4, C5,
            ...
            SELECT A.* FROM (SELECT SUBCLASS_TAB_NAME,
                   ...
                   FACTOR_TRANSPORT,
                   FLAG
            FROM ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_TAB_NAME || '_DIST
            WHERE BATCH_NUMBER = ''' || V_SUBCLASS_BATCH_NUMBER || '''
        ...
        SET V_QUERY_STR = '
            INSERT INTO ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT_DIST_PARALLEL (SUBCLASS_TAB_NAME, BATCH_NUMBER,
                                                                                    UPDATE_DATE, MAT_NO, MAT_TRACK_NO,
                                                                                    MAT_SEQ_NO, FAMILY_CODE, UNIT_CODE,
                                                                                    UNIT_NAME, PRODUCT_CODE,
                                                                                    PRODUCT_NAME, TYPE_CODE, TYPE_NAME,
                                                                                    ITEM_CODE, ITEM_NAME, VALUE,
                                                                                    UNITM_AC, PRODUCT_VALUE, UNIT_COST,
                                                                                    C1, C2, C3, C4, C5, FACTOR_DIRECT,
                                                                                    FACTOR_INDIRECT, FACTOR_TRANSPORT,
                                                                                    FLAG)
            SELECT A.* FROM (SELECT SUBCLASS_TAB_NAME,
                   BATCH_NUMBER,
                   UPDATE_DATE,
                          ...
                          FROM (SELECT A.*
                                FROM (SELECT *
                                   ...
                                   INNER JOIN (SELECT SUBCLASS_TAB_NAME,
                                               ...
                                               GROUP BY SUBCLASS_TAB_NAME, BATCH_NUMBER, UPDATE_DATE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE,
                                                        UNIT_CODE,
                                                        UNIT_NAME, PRODUCT_CODE, PRODUCT_NAME, PRODUCT_VALUE) B
                                              ON A.MAT_TRACK_NO = B.MAT_TRACK_NO AND A.ITEM_CODE = B.PRODUCT_CODE)
                    GROUP BY SUBCLASS_TAB_NAME, BATCH_NUMBER, UPDATE_DATE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE, UNIT_CODE, UNIT_NAME,
                          ...
                          FROM (SELECT *
                                FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESOURCE_MAIN) A
                                   ...
                                   INNER JOIN (SELECT SUBCLASS_TAB_NAME,
                                               ...
                                               FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT_DIST_PARALLEL
                                               GROUP BY SUBCLASS_TAB_NAME, BATCH_NUMBER, UPDATE_DATE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE,
                                                        UNIT_CODE, UNIT_NAME, PRODUCT_CODE, PRODUCT_NAME, PRODUCT_VALUE) C
                                              ON B.MAT_TRACK_NO = C.MAT_TRACK_NO AND B.PREV_FAMILY_CODE = C.FAMILY_CODE)
                    GROUP BY SUBCLASS_TAB_NAME, BATCH_NUMBER, UPDATE_DATE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE, UNIT_CODE, UNIT_NAME,
                             PRODUCT_CODE, PRODUCT_NAME, TYPE_CODE, TYPE_NAME, ITEM_CODE, ITEM_NAME, UNITM_AC, PRODUCT_VALUE, FACTOR_DIRECT,
                             FACTOR_INDIRECT, FACTOR_TRANSPORT, FLAG';
    ...
    SET V_QUERY_STR = '
        ...
        WHERE SUBCLASS_TAB_NAME = ''' || V_SUBCLASS_TAB_NAME || '''
          AND BATCH_NUMBER = ''' || V_SUBCLASS_BATCH_NUMBER || '''
          AND COMPANY_CODE = ''' || V_COMPANY_CODE || '''
    ...
    SET V_QUERY_STR = '
    INSERT INTO ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_TAB_NAME || '_DIST
    (REC_ID, SUBCLASS_TAB_NAME, COMPANY_CODE, BATCH_NUMBER, MAIN_CAT_BATCH_NUMBER, UPDATE_DATE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO,
    ...
    SET V_QUERY_STR = '
    INSERT INTO ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_TAB_NAME || '_DIST_PARALLEL
    (REC_ID, SUBCLASS_TAB_NAME, COMPANY_CODE, BATCH_NUMBER, MAIN_CAT_BATCH_NUMBER, UPDATE_DATE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO,
     FAMILY_CODE, UNIT_CODE, UNIT_NAME, PRODUCT_CODE, PRODUCT_NAME, TYPE_CODE, TYPE_NAME, ITEM_CODE, ITEM_NAME, VALUE,
    ...
    SET V_QUERY_STR = '
    INSERT INTO ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_TAB_NAME || '
    (REC_ID, SUBCLASS_TAB_NAME, COMPANY_CODE, BATCH_NUMBER,
     MAIN_CAT_BATCH_NUMBER, UPDATE_DATE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO,
