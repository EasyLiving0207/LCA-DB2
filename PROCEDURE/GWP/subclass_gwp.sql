CREATE OR REPLACE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_GWP(IN V_COMPANY_CODE VARCHAR(4),
    ...
    SPECIFIC P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_GWP
    LANGUAGE SQL
    NOT DETERMINISTIC
    EXTERNAL ACTION
    MODIFIES SQL DATA
...
BEGIN
    ...
    DECLARE V_ROUTINE_NAME VARCHAR(128) DEFAULT 'P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_GWP'; --存储过程名
    ...
    DECLARE TAR_TAB1 VARCHAR(50) DEFAULT 'T_ADS_FACT_LCA_SUBCLASS_GWP_RESULT'; --目标表名
    DECLARE SRC_TAB_NAME1 VARCHAR(32) DEFAULT ' '; --源表SCHEMA.表名
    DECLARE V_TMP_SCHEMA VARCHAR(32) DEFAULT 'BG00MAC102'; --临时表SCHEMA
    DECLARE V_TMP_TAB VARCHAR(50) DEFAULT 'T_ADS_TEMP_LCA_SUBCLASS_SEQ_CALC_GWP'; --临时表名
    DECLARE V_PNAME VARCHAR(50) DEFAULT 'P_ADS_FACT_LCA_SUBCLASS_SEQ_CALC_GWP'; --存储过程名
...
--     DECLARE CONTINUE HANDLER FOR NOT FOUND SET DONE = 1;
    ...
    SET V_QUERY_STR = '
                SELECT *,
                CASE WHEN LCI_ELEMENT_NAME = ''全球变暖潜力(GWP100):合计'' THEN ''GWP''
                     WHEN LCI_ELEMENT_NAME = ''全球变暖潜力(GWP100):化石能源'' THEN ''GWP_FOSSIL_ENERGY''
                     WHEN LCI_ELEMENT_NAME = ''全球变暖潜力(GWP100):生物质'' THEN ''GWP_BIOMASS''
                     WHEN LCI_ELEMENT_NAME = ''全球变暖潜力(GWP100):土地利用和土地用途改变'' THEN ''GWP_LAND_USE''
                     END AS LCI_ELEMENT
                ...
                AND LCI_ELEMENT_NAME IN (''全球变暖潜力(GWP100):合计'', ''全球变暖潜力(GWP100):化石能源'',
                                            ''全球变暖潜力(GWP100):生物质'', ''全球变暖潜力(GWP100):土地利用和土地用途改变'')
        ';
    ...
    SET V_QUERY_STR = '
        SELECT BATCH_NUMBER,
               ...
               ITEM_NAME,
               CASE
                   WHEN UNITM_AC = ''万度'' THEN ABS(VALUE) * 10000
                   WHEN UNITM_AC = ''吨'' THEN ABS(VALUE) * 1000
                   WHEN UNITM_AC = ''千立方米'' THEN ABS(VALUE) * 1000
                   ELSE ABS(VALUE)
                   END
                    AS VALUE,
               CASE
    ...
    SET V_QUERY_STR = '
         SELECT ITEM_CODE, LCI_ELEMENT, LCI_ELEMENT_NAME, CAST(LCI_ELEMENT_VALUE AS DOUBLE) AS GWP_FCP
    ...
    SET V_QUERY_STR = '
         SELECT ITEM_CODE, LCI_ELEMENT, LCI_ELEMENT_NAME, CAST(LCI_ELEMENT_VALUE AS DOUBLE) AS GWP_SY
                   FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_GWP
                   WHERE FLAG = ''SY''
                     AND LCI_ELEMENT_VALUE != 0
     ';
    ...
    --直排系数
    SET V_QUERY_STR = '
         SELECT A.ITEM_CODE, A.ITEM_NAME, B.DISCH_COEFF
    ...
    SELECT DISTINCT PROC_KEY,
        ...
        WHERE PRODUCT_CODE NOT IN (SELECT DISTINCT PRODUCT_CODE FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_MAIN_PRODUCT)
    ';
    ...
    SET V_QUERY_STR = '
        SELECT A.*,
               CASE
                   WHEN A.LCI_ELEMENT IN (''GWP'', ''GWP_FOSSIL_ENERGY'') THEN COALESCE(A.UNIT_COST * B.DISCH_COEFF, 0)
                   ELSE 0 END                                 AS C1,
               COALESCE(A.UNIT_COST * C.GWP_FCP, 0)           AS C2,
               COALESCE(A.UNIT_COST * D.GWP_SY, 0)            AS C3,
               0                                              AS C4,
               COALESCE(A.UNIT_COST * E.LCI_ELEMENT_VALUE, 0) AS C5,
               B.DISCH_COEFF                                  AS FACTOR_DIRECT,
               COALESCE(C.GWP_FCP, D.GWP_SY)                  AS FACTOR_INDIRECT,
               E.LCI_ELEMENT_VALUE                            AS FACTOR_TRANSPORT,
               ''DIRECT''                                       AS FLAG
        FROM (SELECT *
              FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESOURCE
                       LEFT JOIN (SELECT DISTINCT LCI_ELEMENT, LCI_ELEMENT_NAME
                                  FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ENERGY_RESULT) ON 1 = 1) A
                 LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_DIRECT B
                           ON A.ITEM_CODE = B.ITEM_CODE
                 LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_FCP C
                           ON A.ITEM_CODE = C.ITEM_CODE AND A.LCI_ELEMENT = C.LCI_ELEMENT
                 LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_SY D
                           ON A.ITEM_CODE = D.ITEM_CODE AND A.LCI_ELEMENT = D.LCI_ELEMENT
                 LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_TRANSPORT E
                           ON A.ITEM_CODE = E.ITEM_CODE AND A.LCI_ELEMENT = E.LCI_ELEMENT
        UNION
        SELECT A.*,
               CASE
                   WHEN A.LCI_ELEMENT IN (''GWP'', ''GWP_FOSSIL_ENERGY'') THEN COALESCE(-A.UNIT_COST * B.DISCH_COEFF, 0)
               ...
               0                                               AS C2,
               0                                               AS C3,
               COALESCE(-A.UNIT_COST * C.LCI_ELEMENT_VALUE, 0) AS C4,
               0                                               AS C5,
               B.DISCH_COEFF                                   AS FACTOR_DIRECT,
               C.LCI_ELEMENT_VALUE                             AS FACTOR_INDIRECT,
               NULL                                            AS FACTOR_TRANSPORT,
               ''DIRECT''                                        AS FLAG
        FROM (SELECT *
              FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PRODUCT_BY_PRODUCT
                       LEFT JOIN (SELECT DISTINCT LCI_ELEMENT, LCI_ELEMENT_NAME
                                  FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ENERGY_RESULT) ON 1 = 1) A
                 LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_DIRECT B
                           ON A.ITEM_CODE = B.ITEM_CODE
                 LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_GWP C
                           ON A.ITEM_CODE = C.ITEM_CODE AND A.LCI_ELEMENT = C.LCI_ELEMENT
        UNION
        SELECT A.*,
        ...
        FROM (SELECT *
              FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESOURCE_ENERGY
                 ...
                 LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ENERGY_RESULT B
                           ON A.ITEM_CODE = B.PRODUCT_CODE AND A.LCI_ELEMENT = B.LCI_ELEMENT
        UNION
        SELECT A.*,
               CASE
                   WHEN A.LCI_ELEMENT IN (''GWP'', ''GWP_FOSSIL_ENERGY'') THEN COALESCE(A.UNIT_COST * B.DISCH_COEFF, 0)
                    ...
                    FROM (SELECT A.*
                          FROM (SELECT *
                             ...
                             INNER JOIN (SELECT MAT_TRACK_NO,
                                   ...
                                   GROUP BY MAT_TRACK_NO, FAMILY_CODE, UNIT_CODE, PRODUCT_CODE, LCI_ELEMENT, LCI_ELEMENT_NAME) B
                                  ON A.MAT_TRACK_NO = B.MAT_TRACK_NO AND A.ITEM_CODE = B.PRODUCT_CODE';
                PREPARE stmt FROM V_QUERY_STR;
                EXECUTE stmt;
            END IF;
                                ...
                                FROM (SELECT *
                                   ...
                                   INNER JOIN (SELECT SUBCLASS_TAB_NAME,
                                               ...
                                               GROUP BY SUBCLASS_TAB_NAME, BATCH_NUMBER, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE,
                                                        UNIT_CODE, UNIT_NAME, PRODUCT_CODE, PRODUCT_NAME, PRODUCT_VALUE, LCI_ELEMENT,
                                                        LCI_ELEMENT_NAME) B
                                              ON A.MAT_TRACK_NO = B.MAT_TRACK_NO AND A.ITEM_CODE = B.PRODUCT_CODE)
                    GROUP BY SUBCLASS_TAB_NAME, BATCH_NUMBER, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE, UNIT_CODE, UNIT_NAME,
                             PRODUCT_CODE, PRODUCT_NAME, TYPE_CODE, TYPE_NAME, ITEM_CODE, ITEM_NAME, UNITM_AC, PRODUCT_VALUE, LCI_ELEMENT,
                             LCI_ELEMENT_NAME, FACTOR_DIRECT, FACTOR_INDIRECT, FACTOR_TRANSPORT, FLAG';
            PREPARE stmt FROM V_QUERY_STR;
            EXECUTE stmt;
                ...
                FROM (SELECT *
                      FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESOURCE_MAIN) A
                         ...
                         INNER JOIN (SELECT MAT_TRACK_NO,
                                            ...
                                            SUM(C5) AS C5
                                     FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT_DIST
                                     GROUP BY MAT_TRACK_NO, FAMILY_CODE, UNIT_CODE, PRODUCT_CODE, LCI_ELEMENT,
                                            LCI_ELEMENT_NAME) C
    ...
    DELETE
    FROM BG00MAC102.T_ADS_FACT_LCA_SUBCLASS_GWP_RESULT
    WHERE SUBCLASS_TAB_NAME = V_SUBCLASS_TAB_NAME
      AND BATCH_NUMBER = V_SUB_BATCH_NUMBER
      AND COMPANY_CODE = V_COMPANY_CODE;

    DELETE
    FROM BG00MAC102.T_ADS_FACT_LCA_SUBCLASS_GWP_RESULT_DIST
    WHERE SUBCLASS_TAB_NAME = V_SUBCLASS_TAB_NAME
      AND BATCH_NUMBER = V_SUB_BATCH_NUMBER
      AND COMPANY_CODE = V_COMPANY_CODE;

    DELETE
    FROM BG00MAC102.T_ADS_FACT_LCA_SUBCLASS_GWP_RESULT_DIST_PARALLEL
    WHERE SUBCLASS_TAB_NAME = V_SUBCLASS_TAB_NAME
      AND BATCH_NUMBER = V_SUB_BATCH_NUMBER
      AND COMPANY_CODE = V_COMPANY_CODE;
    ...
    SET V_QUERY_STR = '
    INSERT INTO BG00MAC102.T_ADS_FACT_LCA_SUBCLASS_GWP_RESULT_DIST
    (REC_ID, SUBCLASS_TAB_NAME, COMPANY_CODE, BATCH_NUMBER, MAIN_CAT_BATCH_NUMBER, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO,
     ...
     UNITM_AC, PRODUCT_VALUE, UNIT_COST, LCI_ELEMENT, LCI_ELEMENT_NAME, C1, C2, C3, C4, C5, FACTOR_DIRECT, FACTOR_INDIRECT, FACTOR_TRANSPORT, FLAG,
    ...
    SET V_QUERY_STR = '
    INSERT INTO BG00MAC102.T_ADS_FACT_LCA_SUBCLASS_GWP_RESULT_DIST_PARALLEL
    (REC_ID, SUBCLASS_TAB_NAME, COMPANY_CODE, BATCH_NUMBER, MAIN_CAT_BATCH_NUMBER, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO,
     FAMILY_CODE, UNIT_CODE, UNIT_NAME, PRODUCT_CODE, PRODUCT_NAME, TYPE_CODE, TYPE_NAME, ITEM_CODE, ITEM_NAME, VALUE,
     UNITM_AC, PRODUCT_VALUE, UNIT_COST, LCI_ELEMENT, LCI_ELEMENT_NAME, DEPT_NAME, DEPT_CODE, DEPT_MID_NAME, C1, C2, C3, C4, C5, C_CYCLE, FACTOR_DIRECT,
    ...
    SET V_QUERY_STR = '
    INSERT INTO BG00MAC102.T_ADS_FACT_LCA_SUBCLASS_GWP_RESULT
    (REC_ID, SUBCLASS_TAB_NAME, COMPANY_CODE, BATCH_NUMBER,
     ...
     PRODUCT_VALUE, LCI_ELEMENT, LCI_ELEMENT_NAME, C1, C2, C3, C4, C5, C_INSITE, C_OUTSITE,
    ...
    FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_RESULT_DIST
    GROUP BY SUBCLASS_TAB_NAME, BATCH_NUMBER, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE, UNIT_CODE, UNIT_NAME,
             PRODUCT_CODE, PRODUCT_NAME, PRODUCT_VALUE, LCI_ELEMENT, LCI_ELEMENT_NAME
