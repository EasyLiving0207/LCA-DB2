...
BEGIN
    ...
    SET V_QUERY_STR = 'MERGE INTO ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATA A
                       USING (SELECT DISTINCT A.INDEX_CODE, A.PREV_INDEX_CODE, B.PRODUCT_CODE AS PREV_PRODUCT
    ...
    SET V_QUERY_STR = 'MERGE INTO ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATA A
                       USING (SELECT A.*, B.PRODUCT_CODE FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATE_BATCH A
                            JOIN (SELECT DISTINCT BATCH_NUMBER, PRODUCT_CODE
                              FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ENERGY_RESULT) B
                            ON A.BATCH_NUMBER = B.BATCH_NUMBER) B
                       ON A.FLAG = ''RESOURCE'' AND A.ITEM_CODE = B.PRODUCT_CODE AND A.UPDATE_DATE = B.UPDATE_DATE
                       WHEN
                           MATCHED THEN
                           UPDATE
                           SET A.FLAG = ''ENERGY''';
             ...
             DIST_RESOURCE AS (
                 SELECT A.*,
                   ...
                   JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_FACTOR_MAT B
                     ON B.LCI_ELEMENT_CODE = ''GWP-total''
