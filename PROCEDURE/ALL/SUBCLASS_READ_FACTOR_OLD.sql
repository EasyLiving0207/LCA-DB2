...
BEGIN
    ...
    ----------------------------------------------------------------------------
    SET V_QUERY_STR =
            'SELECT A.DATA_CODE AS ITEM_CODE, ' ||
            '       A.ITEM_NAME, ' ||
            '       B.SOURCE_ITEM_CODE, ' ||
            ...
            'FROM BG00MAC102.T_ADS_WH_PROC_BACKGROUND_DATA_CONTRAST A ' ||
            'JOIN BG00MAC102.T_ADS_WH_ZP_FACTOR_LIBRARY B ' ||
            '  ON A.UUID = B.ITEM_CODE ' ||
            'WHERE A.BASE_CODE  = ''' || V_COMPANY_ESC || ''' ' ||
    ...
    ----------------------------------------------------------------------------
    -- UUID sets: compute once and materialize as needed
    -- UUID_FCP: max(uuid) per data_code for FCP
    ----------------------------------------------------------------------------
    SET V_QUERY_STR =
            'SELECT DATA_CODE, MAX(UUID) AS UUID ' ||
            'FROM BG00MAC102.T_ADS_WH_PROC_BACKGROUND_DATA_CONTRAST ' ||
            'WHERE FLAG = ''FCP'' ' ||
            '  AND BASE_CODE  = ''' || V_COMPANY_ESC || ''' ' ||
            '  AND START_TIME = ''' || V_YEAR_ESC    || ''' ' ||
            'GROUP BY DATA_CODE';

    CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'UUID_FCP', V_QUERY_STR);

    ----------------------------------------------------------------------------
    -- UUID_SY: SY preferred; fallback to FCP for codes not in SY
    -- (rewritten without NOT IN; uses anti-join)
    ----------------------------------------------------------------------------
    SET V_QUERY_STR =
            'WITH SY AS ( ' ||
            '  SELECT DATA_CODE, MAX(UUID) AS UUID ' ||
            ...
            '  GROUP BY DATA_CODE ' ||
            ') ' ||
            'SELECT SY.DATA_CODE, SY.UUID ' ||
            'FROM SY ' ||
            'UNION ALL ' ||
            'SELECT FCP.DATA_CODE, FCP.UUID ' ||
            'FROM FCP ' ||
            'LEFT JOIN SY ON SY.DATA_CODE = FCP.DATA_CODE ' ||
            'WHERE SY.DATA_CODE IS NULL';

    CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'UUID_SY', V_QUERY_STR);

    ----------------------------------------------------------------------------
    -- UUID_STREAM (LCI)
    ----------------------------------------------------------------------------
    SET V_QUERY_STR =
            'SELECT DATA_CODE, UUID ' ||
            'FROM BG00MAC102.T_ADS_WH_PROC_BACKGROUND_DATA_CONTRAST ' ||
            'WHERE FLAG = ''LCI'' ' ||
            '  AND BASE_CODE  = ''' || V_COMPANY_ESC || ''' ' ||
            '  AND START_TIME = ''' || V_YEAR_ESC    || '''';
    ...
    SET V_QUERY_STR =
            ...
            '    AND FLAG = ''STREAM'' ' ||
            ') ' ||
            'SELECT A.DATA_CODE, A.UUID AS STREAM_ID, ' ||
    ...
    ----------------------------------------------------------------------------
    SET V_QUERY_STR =
            'SELECT DATA_CODE AS ITEM_CODE, LCI_ELEMENT_CODE, LCI_ELEMENT_NAME, LCI_ELEMENT_VALUE ' ||
    ...
    SET V_QUERY_STR =
            ...
            'FROM TRANS T ' ||
            'JOIN FACTOR_TRANSPORT1 F ON T.DATA_CODE = F.NAME ' ||
    ...
    SET V_QUERY_STR =
            ...
            '  SELECT ITEM_CODE FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_ITEM_BASE ' ||
            ') ' ||
            'SELECT A.DATA_CODE AS ITEM_CODE, B.MAT_NAME, B.LCI_ELEMENT_CODE, B.LCI_ELEMENT_NAME, B.LCI_ELEMENT_VALUE ' ||
            'FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_UUID_FCP A ' ||
            'JOIN MAT_LIB B ON A.UUID = B.UUID ' ||
            'JOIN LCI C ON B.LCI_ELEMENT_CODE = C.LCI_ELEMENT_CODE ' ||
            'JOIN ITEMS I ON A.DATA_CODE = I.ITEM_CODE';
    ...
    SET V_QUERY_STR =
            ...
            ') ' ||
            'SELECT A.DATA_CODE AS ITEM_CODE, B.MAT_NAME, B.LCI_ELEMENT_CODE, B.LCI_ELEMENT_NAME, B.LCI_ELEMENT_VALUE ' ||
            'FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_UUID_SY A ' ||
            'JOIN MAT_LIB B ON A.UUID = B.UUID ' ||
            'JOIN LCI C ON B.LCI_ELEMENT_CODE = C.LCI_ELEMENT_CODE ' ||
            'JOIN ITEMS I ON A.DATA_CODE = I.ITEM_CODE';
