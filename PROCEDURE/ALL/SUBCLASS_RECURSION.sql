CREATE OR REPLACE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_RECURSION(
    IN V_PCR VARCHAR(6),
    ...
    SPECIFIC P_ADS_FACT_LCA_SUBCLASS_RECURSION
    LANGUAGE SQL
    NOT DETERMINISTIC
    EXTERNAL ACTION
    MODIFIES SQL DATA
...
BEGIN
    ...
    INTO V_TAB_COUNT
    FROM SYSCAT.TABLES
    WHERE TABNAME = V_SUBCLASS_RESULT_TAB_NAME
      AND TABSCHEMA = V_TMP_SCHEMA;

    IF V_TAB_COUNT = 0 THEN
        EXECUTE IMMEDIATE '
        CREATE TABLE ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_TAB_NAME || '
        (
            REC_ID                VARCHAR(64),
            PCR                   VARCHAR(32),
    ...
    ELSE
        ...
        IF V_START_YM IS NOT NULL THEN
            SET V_QUERY_STR = V_QUERY_STR || ' AND UPDATE_DATE >= ''' || V_START_YM || '''';
        END IF;
        IF V_END_YM IS NOT NULL THEN
            SET V_QUERY_STR = V_QUERY_STR || ' AND UPDATE_DATE <= ''' || V_END_YM || '''';
        END IF;
    ...
    INTO V_TAB_COUNT
    FROM SYSCAT.TABLES
    WHERE TABNAME = V_SUBCLASS_RESULT_DIST_TAB_NAME
      AND TABSCHEMA = V_TMP_SCHEMA;

    IF V_TAB_COUNT = 0 THEN
        EXECUTE IMMEDIATE '
        CREATE TABLE ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_DIST_TAB_NAME || '
        (
            REC_ID                VARCHAR(64),
            PCR                   VARCHAR(32),
            FACTOR_VERSION        VARCHAR(100),
            ...
            ORGANIZE BY ROW';
    ELSE
        SET V_QUERY_STR = 'DELETE FROM ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_DIST_TAB_NAME || ' WHERE
                       SUBCLASS_TAB_NAME = ''' || V_SUBCLASS_TAB_NAME || '''
                       AND PCR = ''' || V_PCR || '''
                       AND COMPANY_CODE = ''' || V_COMPANY_CODE || '''
                       AND FACTOR_VERSION = ''' || V_FACTOR_VERSION || '''';
    ...
    SET V_QUERY_STR = 'SELECT DISTINCT MIN(RANK) AS MIN_RANK
                       FROM ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_PROC_SEQ';
    CALL BG00MAC102.P_EXEC_INTO_C(V_QUERY_STR, V_MIN_RANK);

    SET V_RANK = V_MIN_RANK;
    ...
    WHILE V_RANK <= V_MAX_RANK
        DO
            ...
            SET V_QUERY_STR = 'INSERT INTO ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_TAB_NAME ||
                              ' (REC_ID, PCR, FACTOR_VERSION, ' ||
                              'COMPANY_CODE, AUTO_BATCH, BATCH_NUMBER, SUBCLASS_TAB_NAME, UPDATE_DATE, INDEX_CODE, MAT_NO, MAT_TRACK_NO, ' ||
                       ...
                       FROM (SELECT A.*, B.LCI_ELEMENT_CNAME, C.DEPT_NAME, C.DEPT_CODE, C.DEPT_MID_NAME,
                              ...
                              FROM SESSION.RESULT_BUFFER A
                                       LEFT JOIN (SELECT LCI_ELEMENT_CODE, LCI_ELEMENT_CNAME
                                                  FROM ' || V_TMP_SCHEMA || '.' || V_FACTOR_LIST || ' WHERE VERSION = ''' ||
                              V_FACTOR_VERSION || ''') B
                                       ...
                                       LEFT JOIN ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DATE_BATCH E
                                          ON D.UPDATE_DATE = E.UPDATE_DATE)
            ...
            SET V_QUERY_STR = 'INSERT INTO ' || V_TMP_SCHEMA || '.' || V_SUBCLASS_RESULT_DIST_TAB_NAME || ' (REC_ID, PCR, ' ||
                              'FACTOR_VERSION, COMPANY_CODE, AUTO_BATCH, BATCH_NUMBER, SUBCLASS_TAB_NAME, ' ||
