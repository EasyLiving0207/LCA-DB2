CREATE OR REPLACE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_MONTH(IN V_COMPANY_CODE VARCHAR(4),
                                                                                   ...
                                                                                   IN V_RANK_LIMIT BIGINT)
    SPECIFIC P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_MONTH
    LANGUAGE SQL
    NOT DETERMINISTIC
    EXTERNAL ACTION
    MODIFIES SQL DATA
...
BEGIN
    ...
    DECLARE V_ROUTINE_NAME VARCHAR(128) DEFAULT 'P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_MONTH'; --存储过程名
    ...
    DECLARE V_TMP_TAB VARCHAR(50) DEFAULT 'T_ADS_TEMP_LCA_SUBCLASS_EPD_CONS_CALC_MONTH'; --临时表名
    DECLARE V_PNAME VARCHAR(50) DEFAULT 'P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_MONTH'; --存储过程名
...
--     DECLARE CONTINUE HANDLER FOR NOT FOUND SET DONE = 1;
    ...
    IF V_COMPANY_CODE NOT IN ('TA', 'BSZG') THEN
        RETURN;
    END IF;
    ...
    IF V_COMPANY_CODE = 'TA' THEN
        SET V_QUERY_STR = 'UPDATE ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_TEMP_DATA
                        ...
                        WHERE UNIT_CODE LIKE ''%BOF''
                          AND TYPE_CODE != ''04''';
    ELSEIF V_COMPANY_CODE = 'BSZG' THEN
        SET V_QUERY_STR = 'UPDATE ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_TEMP_DATA
                        SET VALUE = VALUE * 0.9877
            ...
            SET V_QUERY_STR = 'INSERT INTO ' || V_TMP_SCHEMA || '.' || V_TMP_TAB || '_DIST (SUBCLASS_TAB_NAME, UPDATE_DATE, MAT_NO, MAT_TRACK_NO,
                               ...
                               WITH RANKED AS (SELECT MAT_TRACK_NO, FAMILY_CODE, PREV_FAMILY_CODE
                                    ...
                                    DIST_AGG AS (SELECT MAT_TRACK_NO,
                                                 ...
                                                 GROUP BY MAT_TRACK_NO, FAMILY_CODE, LCI_ELEMENT_CODE)
                               ...
                               GROUP BY B.SUBCLASS_TAB_NAME, B.UPDATE_DATE, B.MAT_NO, B.MAT_TRACK_NO, B.MAT_SEQ_NO,
                                        ...
                                        B.PRODUCT_VALUE, D.LCI_ELEMENT_CODE';
            PREPARE stmt FROM V_QUERY_STR;
            EXECUTE stmt;
