CREATE OR REPLACE PROCEDURE BG00MAC102.P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_AUTO_BATCH_AUTH(IN V_COMPANY_CODE VARCHAR(4),
    ...
    SPECIFIC P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_AUTO_BATCH_AUTH
    LANGUAGE SQL
    NOT DETERMINISTIC
    EXTERNAL ACTION
    MODIFIES SQL DATA
...
BEGIN
    ...
    DECLARE V_ROUTINE_NAME VARCHAR(128) DEFAULT 'P_ADS_FACT_LCA_SUBCLASS_EPD_CONS_CALC_AUTO_BATCH_AUTH'; --存储过程名
    ...
    DECLARE V_TMP_TAB VARCHAR(53) DEFAULT 'T_ADS_TEMP_LCA_SUBCLASS_EPD_CONS_CALC_AUTO_BATCH_AUTH'; --临时表名
    ...
    ELSE
        SET V_QUERY_STR = 'WITH UPDATE_DATE AS (SELECT DISTINCT UPDATE_DATE FROM ' || V_TMP_SCHEMA || '.' ||
                          V_SUBCLASS_TAB_NAME || '),
                             ...
                             BATCH AS (SELECT * FROM BATCH_RANK WHERE BATCH_RANK = 1)';
    END IF;
    IF V_DEFAULT_BATCH IS NOT NULL THEN
        SET V_QUERY_STR = V_QUERY_STR || '
                        SELECT A.UPDATE_DATE, COALESCE(B.BATCH_NUMBER, ''' || V_DEFAULT_BATCH || ''') AS BATCH_NUMBER
                        FROM UPDATE_DATE A
                                 LEFT JOIN BATCH B ON A.UPDATE_DATE = B.UPDATE_DATE
                        ORDER BY A.UPDATE_DATE';
    ELSE
        SET V_QUERY_STR = V_QUERY_STR || '
                        SELECT A.UPDATE_DATE, B.BATCH_NUMBER
                        FROM UPDATE_DATE A
                                 LEFT JOIN BATCH B ON A.UPDATE_DATE = B.UPDATE_DATE
                        ORDER BY A.UPDATE_DATE';
    END IF;
...
--     CALL BG00MAC102.P_CREATE_TEMP_TABLE(V_TMP_SCHEMA, V_TMP_TAB, 'FACTOR_TRANSPORT', V_QUERY_STR);
    ...
    WHILE V_RANK <= V_MAX_RANK
        DO
            ...
            GROUP BY BATCH_NUMBER, UPDATE_DATE, MAT_NO, MAT_TRACK_NO, MAT_SEQ_NO, FAMILY_CODE, UNIT_CODE, UNIT_NAME,
